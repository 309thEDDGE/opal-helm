...
values.schema.yaml covers values.yaml, and vice versa.

FIXME: It would be nice to run this as part of our CI pipeline to report if
       values.schema.yaml and values.yaml gets out of sync, but first we need to
       address what it means to be out of sync.

       Consider if values.schema.yaml describes extraLabels, and we in this helm chart
...
values_yaml = os.path.join(here_dir, os.pardir, "apps", chart, "values.yaml")
...
def flatten(d, parent_key="", sep="."):
    ...
    so one element returned would for example be "gateway.image.tag".
    """
    items = []
    for k, v in d.items():
...
def run():
    ...
    print(
        "The keys from values.schema.yaml minus those from values.yaml:\n",
